...
def continent_code(continent: str) -> int:
    if continent in CONT_MAPS:
        return CONT_MAPS[continent]
    else:
        return-1

def country_code(continent: str) -> tuple:
    if country in PYCOUNTRY_MAP:
        id_mean = (PYCOUNTRY_MAP[country]["id"],PYCOUNTRY_MAP[country]["mean"])
        return PYCOUNTRY_MAP[country]["id"]["mean"]
    else:
        return (-1, 0)
